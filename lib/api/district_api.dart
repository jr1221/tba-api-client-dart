...
class DistrictApi {
  ...
  DistrictApi([ApiClient? apiClient])
  ...
  Future<Response> getDistrictsByYearWithHttpInfo(int year,
      {String? ifModifiedSince}) async {
    Object? postBody;
  ...
  ///
  /// Gets a list of districts and their corresponding district key, for the given year.
  Future<List<DistrictList>> getDistrictsByYear(int year,
      {String? ifModifiedSince}) async {
    Response response = await getDistrictsByYearWithHttpInfo(year,
        ifModifiedSince: ifModifiedSince);
    if (response.statusCode >= 400) {
      throw ApiException(response.statusCode, _decodeBodyBytes(response));
    } else if (response.body != null) {
